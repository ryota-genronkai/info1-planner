...
import { Trophy, Undo2, ArrowRight, Notebook, BookOpen, Code2, Target, Workflow, Plus, Trash2, ExternalLink, ImageIcon, ChevronRight } from "lucide-react";
...
const PRIMARY = "#00a0e9";   // プライマリー
const SECONDARY = "#ffcf7f"; // セカンダリー

// ヘルパー: 日付→ISO
const toISO = (d: Date) => d.toISOString().slice(0, 10);
const addDays = (dateStr: string, n: number) => { const d = new Date(dateStr); d.setDate(d.getDate() + n); return toISO(d); };
...
type Session = {
  ...
  weeklyStart?: ISODate; // 週間表示の基準（全体戦略開始日の最小値で初期化）
  examType?: ExamType;
  examYear?: ExamYear;
  examLabel?: string;
};
...
type ISODate = string; // "2025-08-19"
...
export default function Info1Planner() {
  const [session, setSession] = useLocalStorage<Session>("info1_planner_v9", initialSession);
  ...
  const solutions: Solution[] = useMemo(() => {
    ...
    if (isUnlearned) { actions.push({ node: "Ov", reason: "未修のため、まずは定義・範囲の把握" }); return actions; }
    if (selectedCauses.length > 0) { for (const c of selectedCauses) actions.push({ node: c.to as NodeKey, reason: c.label }); }
    return actions;
  ...
  // 追加：開始日の初期同期（最小開始日→weeklyStart）
  useEffect(() => {
    const dates = (session.strategy || []).map(s => s.startDate).filter(Boolean) as string[];
    if (dates.length) {
      const min = dates.sort()[0];
      if (min && session.weeklyStart !== min) setSession(s => ({ ...s, weeklyStart: min }));
    }
  }, [session.strategy]);
  ...
  function resetToRetry() {
    setSession(s => ({
      ...
      history: [
        ...s.history,
        { at: new Date().toISOString(), target: s.target, prevScore: s.score, solutions, subject: s.subject, label: s.examLabel, examType: s.examType },
      ],
    }));
    setTab("plan");
  ...
  function addToStrategy(step: Solution) {
    ...
    if (exists) { toast("同じ学習段階は既に全体戦略に入っています"); return; }
    setSession(s => ({
      ...s,
      strategy: [
        ...s.strategy,
        { ...step, at: new Date().toISOString(), subject: s.subject, startDate: toISO(new Date()), monthsRange: null },
      ],
    }));
  ...
  function setWeekCell(index: number, day: number, text: string) {
    setSession(s => ({
      ...
      strategy: s.strategy.map((it, i) => {
        ...
        const cells = { ...(it.weekCells || {}) }; cells[day] = text; return { ...it, weekCells: cells } as StrategyItem;
      })
    }));
  }

  function saveWeekSnapshot() {
    const rows = session.strategy.filter(s => s.weekly).map(st => ({ subject: st.subject, title: NODE_META[st.node]?.title || st.node, cells: st.weekCells || {} }));
    if (rows.length === 0) { toast("週間戦略に項目がありません"); return; }
    setSession(s => ({ ...s, weekSnapshots: [...(s.weekSnapshots || []), { at: new Date().toISOString(), rows }] }));
    toast.success("この週の計画を保存しました（履歴に残りました）");
  }

  function removeStrategy(index: number) { setSession(s => ({ ...s, strategy: s.strategy.filter((_, i) => i !== index) })); }
  function clearStrategy() { setSession(s => ({ ...s, strategy: [] })); }
    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8">
      <div className="rounded-2xl shadow-lg ring-1 ring-black/5 bg-white text-slate-900">
        <header className="flex items-center gap-3 p-4 md:p-6 border-b">
          ...
          <Badge variant="outline" className="ml-auto">v9</Badge>
        </header>
          ...
          <Tabs value={tab} onValueChange={setTab}>
            <TabsList className="grid grid-cols-4 w-full md:w-auto rounded-lg p-1" style={{ background: "#e6f7fd" }}>
              {(["plan", "strategy", "weekly", "history"] as const).map(val => (
                <TabsTrigger key={val} value={val} className="rounded-md"
                  style={{
                    color: tab === val ? "white" : "#036086",
                    background: tab === val ? PRIMARY : "transparent"
                  }}
                >{({ plan: "問題解決", strategy: "全体戦略", weekly: "週間戦略", history: "履歴" } as any)[val]}</TabsTrigger>
              ))}
            </TabsList>
            ...
            <TabsContent value="plan" className="mt-6 space-y-6">
              <Card className="bg-white">
                <CardHeader><CardTitle className="flex items-center gap-2">設定</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      <label className="text-sm text-slate-700">教科</label>
                      <div className="border rounded-lg overflow-hidden bg-white">
                        <select className="w-full p-2 bg-white" value={session.subject} onChange={(e) => setSession(s => ({ ...s, subject: e.target.value as Subject, causes: {} }))}>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="border rounded-lg overflow-hidden bg-white mt-1">
                        <select className="w-full p-2 bg-white" value={session.examYear} onChange={(e) => setSession(s => ({ ...s, examYear: Number(e.target.value) as ExamYear, examLabel: `${e.target.value} ${s.examType || ""}` }))}>
                    ...
                    <div>
                      ...
                      <div className="border rounded-lg overflow-hidden bg-white mt-1">
                        <select className="w-full p-2 bg-white" value={session.examType} onChange={(e) => setSession(s => ({ ...s, examType: e.target.value as ExamType, examLabel: `${s.examYear || ""} ${e.target.value}` }))}>
                  ...
                  {session.score >= session.target ? (
                    <div className="rounded-2xl border p-4" style={{ borderColor: "#e6b800", background: "#fff8db" }}>
                      ...
                      <div className="mt-3 flex gap-2">
                        <Button onClick={resetToRetry} variant="default" className="text-white" style={{ background: PRIMARY }}><Undo2 className="w-4 h-4 mr-1" />再挑戦（過去問へ）</Button>
                        <Button asChild variant="secondary" style={{ background: SECONDARY }}><a href={LINKS.A.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1">赤本ページを開く <ExternalLink className="w-3 h-3" /></a></Button>
                      </div>
                    </div>
                  ) : (
                    <>
                      {isInfo ? (
                        <div className="space-y-3">
                            ...
                            {CAUSE_MAP.filter(c => c.key === "unlearned").map((c) => {
                              ...
                              return (
                                <label key={c.key} className={`flex items-start gap-3 rounded-xl border p-3 cursor-pointer transition ${checked ? "ring-2" : "hover:border-slate-300"}`} style={checked ? { borderColor: PRIMARY, boxShadow: `0 0 0 2px ${PRIMARY}33` } : {}}>
                                  <Checkbox checked={checked} onCheckedChange={(v) => { setSession(s => { const next = { ...s.causes } as Record<string, boolean>; if (v) { next["unlearned"] = true; CAUSE_MAP.forEach(x => { if (x.key !== "unlearned") next[x.key] = false; }); } else { next["unlearned"] = false; } return { ...s, causes: next }; }); }} />
                                  <div className="space-y-1"><div className="flex items-center gap-2"><Icon className="w-4 h-4" /><b>{c.label}</b></div><div className="text-xs text-slate-600">{c.hint}</div></div>
                                </label>
                              );
                            })}
                          </div>
                          ...
                          <div className="grid md:grid-cols-2 gap-3">
                            {CAUSE_MAP.filter(c => c.key !== "unlearned").map((c) => {
                              const Icon = c.icon; const checked = !!session.causes[c.key]; const disabled = !!session.causes["unlearned"]; // 未修ONで無効化
                              return (
                                <label key={c.key} className={`flex items-start gap-3 rounded-xl border p-3 cursor-pointer transition ${checked ? "ring-2" : "hover:border-slate-300"} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`} style={checked ? { borderColor: PRIMARY, boxShadow: `0 0 0 2px ${PRIMARY}33` } : {}}>
                                  <Checkbox checked={checked} disabled={disabled} onCheckedChange={(v) => { setSession(s => { const next = { ...s.causes } as Record<string, boolean>; next[c.key] = !!v; if (v) next["unlearned"] = false; return { ...s, causes: next }; }); }} />
                                  <div className="space-y-1"><div className="flex items-center gap-2"><Icon className="w-4 h-4" /><b>{c.label}</b></div><div className="text-xs text-slate-600">{c.hint}</div></div>
                                </label>
                              );
                            })}
                        ...
                        <div className="rounded-lg border p-3 text-sm text-slate-700 bg-slate-50">情報I を選択すると原因分析が表示されます。</div>
                      )}

                      {/* 解決策：カード全体クリック + 右矢印ボタン */}
                      {solutions?.length > 0 && (
                          ...
                          <CardContent className="space-y-3">
                            <div className="grid md:grid-cols-2 gap-3">
                              {solutions.map((step, i) => {
                                const meta = NODE_META[step.node]; const Icon = meta?.icon || ArrowRight; const link = step.node === "Done" ? undefined : LINKS[step.node as keyof LinkMap];
                                return (
                                  <button key={i} onClick={() => addToStrategy(step)} className="text-left rounded-xl border p-3 bg-white/60 group relative hover:shadow-md transition" style={{ borderColor: "#e2e8f0" }}>
                                    <div className="flex items-center gap-2 mb-1"><Icon className="w-4 h-4" /><b>{meta?.title || step.node}</b><Badge variant="outline" className="ml-auto">{i + 1}</Badge></div>
                                    <div className="text-xs text-slate-600 mb-2">{step.reason}</div>
                                    {link?.img ? (<img src={link.img} alt={`${link.title} cover`} className="w-24 h-28 object-cover rounded-md border mb-2" />) : (<div className="w-24 h-28 flex items-center justify-center border rounded-md text-slate-400 mb-2"><ImageIcon className="w-5 h-5" /></div>)}
                                    {link?.url ? (<span className="text-xs underline inline-flex items-center gap-1">教材: {link.title}</span>) : (<span className="text-xs text-slate-500">教材リンクは任意</span>)}
                            ...
                            <div className="flex flex-wrap gap-2"><Button variant="default" className="text-white" style={{ background: PRIMARY }} onClick={resetToRetry}><Undo2 className="w-4 h-4 mr-1" />過去問に再挑戦</Button></div>
                          </CardContent>
                        </Card>
                      )}
                    </>
            ...
            {/* 全体戦略（スリム行 + 開始日 + 12ヶ月フィル） */}
            <TabsContent value="strategy" className="mt-6">
              <Card className="bg-white">
                <CardHeader><CardTitle className="flex items-center gap-2">全体戦略</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  {session.strategy.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                      {session.strategy.map((st, idx) => (
                        <li key={idx} className="grid items-center gap-2 p-2 rounded-lg border bg-white hover:shadow-sm transition"
                          style={{ gridTemplateColumns: "auto 10rem 1fr 10rem minmax(240px,1fr)", borderColor: "#e2e8f0" }}>
                          ...
                          <div className="truncate text-sm font-medium">{NODE_META[st.node]?.title || st.node} — <span className="font-normal text-slate-600 truncate inline-block max-w-full align-bottom">{st.reason}</span></div>
                          <div className="flex items-center gap-1 text-xs">
                            <span className="text-slate-600">開始日</span>
                            <Input type="date" className="h-7 bg-white" value={st.startDate || session.weeklyStart || toISO(new Date())}
                              onChange={(e) => setSession(s => ({
                                ...s,
                                strategy: s.strategy.map((x, i) => i === idx ? { ...x, startDate: e.target.value } : x),
                            ...
                            {Array.from({ length: 12 }, (_, i) => i + 1).map((m, i) => {
                              ...
                              return (
                                <div
                                  ...
                                  onMouseDown={() => handleMonthMouseDown(idx, col)}
                                  onMouseEnter={() => handleMonthMouseEnter(idx, col)}
                                  style={{
                          ...
                          <label className="col-span-full inline-flex items-center gap-2 text-xs text-slate-700 mt-1">
                            ...
                            <Button size="icon" variant="ghost" className="ml-auto" onClick={() => removeStrategy(idx)}><Trash2 className="w-4 h-4" /></Button>
                          </label>
                        </li>
                      ))}
                    </ul>
                  ...
                  {session.strategy.length > 0 && (
                    <div className="pt-1 flex gap-2"><Button variant="secondary" style={{ background: SECONDARY }} onClick={clearStrategy}><Trash2 className="w-4 h-4 mr-1" />全削除</Button></div>
                  )}
                </CardContent>
              </Card>
            </TabsContent>

            {/* 週間戦略：7列の簡潔ビュー（全体戦略の開始日と同期） */}
            <TabsContent value="weekly" className="mt-6">
              <Card className="bg-white">
                <CardHeader className="flex flex-col gap-2"><CardTitle className="flex items-center gap-2">週間戦略</CardTitle>
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
                    <label className="flex items-center gap-2"><span>週の開始日:</span>
                      <Input type="date" className="h-7 w-[160px] bg-white" value={session.weeklyStart || toISO(new Date())}
                        onChange={(e) => setSession(s => ({ ...s, weeklyStart: e.target.value }))} />
                    </label>
                    <div className="ml-auto text-slate-500">全体戦略の最小開始日と自動同期</div>
                  </div>
                </CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-end gap-2"><Button onClick={saveWeekSnapshot} className="text-white" style={{ background: PRIMARY }}>この週を保存</Button></div>
                </CardContent>
              </Card>
            </TabsContent>
              ...
              <Card className="bg-white">
                <CardHeader>
                  <div className="flex flex-wrap gap-3 p-3 pt-0 text-xs text-slate-600">
                    <div className="border rounded-lg overflow-hidden bg-white">
                      <select className="p-2 bg-white" value={session.examType || ""} onChange={(e) => setSession(s => ({ ...s, examType: e.target.value as ExamType }))}>
                        <option value="">（種別を選択）</option>
                        {["共通テスト 本試験", "共通テスト 追試験", "模試"].map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    <div className="border rounded-lg overflow-hidden bg-white">
                      <select className="p-2 bg-white" value={session.subject} onChange={(e) => setSession(s => ({ ...s, subject: e.target.value as Subject }))}>
                        {SUBJECTS.map(sub => <option key={sub} value={sub}>{sub}</option>)}
                      </select>
                    </div>
                  </div>
                    ...
                    <ul className="space-y-3">
                      {filteredHistory.map((h, idx) => (
                        <li key={idx} className="rounded-xl border p-3 bg-white">
                          <div className="text-sm flex items-center gap-2"><Badge className="bg-slate-100 text-slate-900 border-slate-200">{new Date(h.at).toLocaleString()}</Badge><span className="text-slate-700">目標{h.target}点 → 前回{h.prevScore}点</span></div>
                          <div className="mt-2 grid md:grid-cols-2 gap-2">
                            {h.solutions?.map((st: Solution, i: number) => (
                              <div key={i} className="text-xs text-slate-600 flex items-center gap-2"><ArrowRight className="w-3 h-3" /><span>{NODE_META[st.node]?.title || st.node}</span><span className="opacity-70">— {st.reason}</span></div>
                            ))}
                          </div>
                        </li>
                      ))}
